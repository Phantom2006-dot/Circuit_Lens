...
int main(){
  ...
  server.Post("/v1/snapshot/inspect",[&](const auto& req,auto& res){cv::Mat image;if(!image_from_request(req,image,res))return;const auto markings=engine.extract_markings(image);json candidates=json::array();for(const auto& item:engine.classify_snapshot_component(image))candidates.push_back({{"label",item.label},{"confidence",item.confidence},{"status","review_only"}});reply(res,{{"snapshot_component_candidates",candidates},{"microcontroller_evidence",engine.microcontroller_evidence(markings)},{"recognized_markings",markings},{"snapshot_model_mode",engine.snapshot_component_ready()?"torchscript":"unavailable"},{"guidance","Centre one component in the snapshot. Rankings and package-marking evidence are review-only; verify the full marking, package, schematic, or BOM before use."}});});
