...
json InferenceEngine::hardware_response(const cv::Mat& bgr) {
  const auto components=detect(bgr); const auto predictions=classify(bgr); const auto markings=extract_markings(bgr); std::unordered_map<std::string,float> scores; for(const auto& prediction:predictions)scores[prediction.board_id]=prediction.confidence; const auto direct=direct_board_for(markings); if(!direct.empty())scores[direct]=.99F; json matches=json::array();
